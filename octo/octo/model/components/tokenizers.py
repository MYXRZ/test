...
from depthjax.depth.dpt import DPT_DINOv2_jax
...
class ImageTokenizer(nn.Module):
    ...
    depth_model: Optional[ModuleSpec] = None
    use_depth_encoder: bool = False
    depth_encoder: Optional[ModuleSpec] = None
    ...
    def __call__(
        ...
        if self.use_depth_encoder and self.depth_encoder:
            depth_encoder_def = ModuleSpec.instantiate(self.depth_encoder)()
            enc_inputs_float = jnp.asarray(enc_inputs) / 255.0 
            enc_inputs_float = jnp.float32(enc_inputs_float)
            enc_inputs_float = jax.image.resize(enc_inputs_float, (b * t, int(h*14/16), int(w*14/16), c), 'bicubic')
            mean = jnp.array([0.485, 0.456, 0.406])
            std = jnp.array([0.229, 0.224, 0.225])
            enc_inputs_float = (enc_inputs_float - mean)/std
            depth_encode = depth_encoder_def(enc_inputs_float)
            depth_encode = jnp.reshape(depth_encode, (b, t, depth_encode.shape[-2], depth_encode.shape[-1]))
            depth_encode = nn.Dense(
                features=512,
                name="embedding"
            )(depth_encode)
            ...
            if self.proper_pad_mask:
                ...
                pad_mask = jnp.ones(depth_encode.shape[:-1])
            return TokenGroup(depth_encode, pad_mask)

        if self.use_depth and self.depth_model:
            
            depth_model_def = ModuleSpec.instantiate(self.depth_model)()
            enc_inputs_float = jnp.asarray(enc_inputs) / 255.0 
            enc_inputs_float = jnp.float32(enc_inputs_float)
            enc_inputs_float = jax.image.resize(enc_inputs_float, (b * t, 518, 518, c), 'bicubic')
            mean = jnp.array([0.485, 0.456, 0.406])
            std = jnp.array([0.229, 0.224, 0.225])
            enc_inputs_float = (enc_inputs_float - mean)/std
            depth= depth_model_def(enc_inputs_float)
            depth = jnp.expand_dims(depth, axis=-1)
            depth = jax.image.resize(depth, (b * t, h, w, 1), 'bilinear')
            depth = (depth - depth.min()) / (depth.max() - depth.min()) * 255.0
            enc_inputs = jnp.concatenate([enc_inputs, jnp.uint8(depth)], axis=-1)
        ...
        if tasks and self.task_stack_keys:
            ...
            task_inputs = task_inputs[:, None].repeat(t, axis=1)
            task_inputs = jnp.reshape(task_inputs, (b * t, h, w, task_inputs.shape[-1]))
