...
from depthjax.vit import depth_encoder_loader
...
def get_config(config_string='vit_s'):
    ...
    config["model"]["observation_tokenizers"] = {
        "primary": ModuleSpec.create(
            ...
            depth_encoder=ModuleSpec.create(DinoViT)
        ),
    ...
    config = update_config(
        config,
        optimizer=dict(
            frozen_keys=("*hf_model*","*DinoViT_0*"),
        ),
        dataset_kwargs=dict(
            oxe_kwargs=dict(
        ...
        pretrained_loaders=(
            ...
            ModuleSpec.create(
                depth_encoder_loader,
                path = "/data/lhr/qkj/depthjax_v1_2/jax_Dino_params.pkl",
            ),
        ),
        eval_datasets=["bridge_dataset"],
        save_dir="/data/lhr/qkj/octo-2/octo/checkpoints_2"
    )

    return config

#python scripts/train.py --config scripts/configs/octo_pretrain_config.py --name=octo_2