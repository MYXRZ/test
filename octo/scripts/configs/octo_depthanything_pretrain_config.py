...
from depthjax.depth.dpt import Depth_model_weights_loader
...
def get_config(config_string='vit_s'):
    ...
    config["model"]["observation_tokenizers"] = {
        "primary": ModuleSpec.create(
            ...
            use_depth=True,
            depth_model=ModuleSpec.create(DPT_DINOv2_jax),
        ),
    ...
    config = update_config(
        config,
        optimizer=dict(
            frozen_keys=("*hf_model*","*DPT_DINOv2_jax*"),
        ),
        dataset_kwargs=dict(
            oxe_kwargs=dict(
                data_mix="oxe_magic_soup",
                data_dir="/data/lhr/whw/tfds_datasets/tensorflow_datasets",
        ...
        pretrained_loaders=(
            ...
            ModuleSpec.create(
                Depth_model_weights_loader,
                path = "/data/lhr/qkj/depthjax/jax_depthmodel_params.pkl",
            ),
        ),
        eval_datasets=["bridge_dataset"],
        save_dir="/data/lhr/qkj/octo/checkpoints"
    )

    return config

#python scripts/train.py --config scripts/configs/octo_pretrain_config.py --name=octo